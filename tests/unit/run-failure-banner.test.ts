...
import { getTerminalRunFailureBanner, isTerminalFailedRunStatus } from "@/components/issue-detail/run-failure-banner";
...
describe("run failure banner helpers", () => {
  ...
  it("builds operator-friendly diagnostic copy with context and next action", () => {
    ...
    expect(banner?.metadata).toContainEqual({ label: "Runtime", value: "Codex local bridge · codex-app-server" });
    expect(banner?.metadata).toContainEqual({ label: "Run", value: "run_1234567890abcdef" });
    expect(banner?.recommendation).toContain("Kick");
  });
