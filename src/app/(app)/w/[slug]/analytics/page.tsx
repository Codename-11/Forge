...
export default function AnalyticsPage() {
  ...
  return (
    <>
      <Topbar title="Analytics" subtitle="Throughput, cycle time, SLA, dispatch" />
  ...
  return (
    <>
      ...
      <section className="mt-6 grid grid-cols-2 gap-4">
        ...
        <Card
          title="SLA breaches"
          tone={breaches && breaches.length > 0 ? "warn" : undefined}
        >
...
function DispatchTab() {
  const { data: summary } = trpc.analytics.dispatch.summary.useQuery({});
    ...
    <>
      <section className="grid grid-cols-4 gap-3">
        <Stat
          ...
          value={summary?.totals.assignments ?? "—"}
        />
        <Stat
          label="Mean TTFA"
          value={formatDuration(summary?.totals.meanTimeToFirstAction)}
        />
        <Stat
          label="Mean TTC"
          value={formatDuration(summary?.totals.meanTimeToCompletion)}
        />
        <Stat
          label="Completed (7d)"
          value={summary?.totals.throughputLast7d ?? "—"}
        />
      </section>
      ...
      <section className="mt-4">
        <Card title="Per-agent">
          <table className="w-full text-xs">
            <thead className="text-left text-[10px] uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th>TTC</th>
                <th>Done 7d</th>
                <th title="Excluded assignments: no follow-up / still open">
                  Excl.
                </th>
              </tr>
            </thead>
          ...
          <p className="mt-3 text-[10px] leading-relaxed text-muted-foreground">
            Excluded from the means: assignments with no follow-up event
            (TTFA) and assignments whose issue is still open (TTC). The
            per-agent row surfaces them under the Excl. column.
          </p>
        </Card>
      </section>
    </>
...
function Timeseries({
  ...
  const max = Math.max(
    1,
    ...data.map((d) => Math.max(d.assignments, d.completions)),
  );
...
function ModeBar({
  ...
  const total = dist
    ? entries.reduce((acc, [k]) => acc + (dist[k] ?? 0), 0)
    : 0;
  if (total === 0) {
    return (
      <div className="text-xs text-muted-foreground">
        No assignments in the window.
      </div>
    );
  }
  return (
...
function Card({
  ...
  return (
    <div
      className={`rounded-lg border border-border bg-card/40 p-4 ${className ?? ""}`}
    >
      <div
