    ...
    <div className="mx-auto w-full max-w-[820px] px-8 py-6">
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Shipped changes show up here, newest first."
        />
      ) : (
        <div className="flex flex-col gap-8">
          {entries.map((entry, i) => {
            ...
            const title =
              dash >= 0 ? entry.heading.slice(dash + 3).trim() : (entry.date ?? entry.version);
            return (
                  ...
                  {GROUPS.map((g) => {
                    ...
                    return (
                      <div key={g.type}>
                        <div
                          className={cn(
                            "text-meta mb-1.5 inline-flex items-center gap-1.5 font-medium",
                            g.cls,
                          )}
                        >
                          <Ico size={12} />
