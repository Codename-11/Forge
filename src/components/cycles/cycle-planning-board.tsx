...
/**
 * Cycle planning kanban. Columns are workspace statuses; cards are issues
 * pinned to this cycle. Drag between columns to change status; drag from
 * the backlog side panel into any column to plan the issue into the cycle.
 */
  ...
  const update = trpc.issue.update.useMutation({
    onMutate: async ({ id, statusId }) => {
      ...
      utils.issue.list.setData(
        { cycleId, includeDone: true, limit: 100 },
        (old) => {
          if (!old) return old;
          return {
            ...old,
            items: old.items.map((i) =>
              i.id === id && statusId
                ? {
                    ...
                    statusId,
                    status:
                      statuses?.find((s) => s.id === statusId) ?? i.status,
                  }
                : i,
            ),
          };
