        ...
        {showFilters || viewAllHref ? (
          <div className="ml-auto flex shrink-0 items-center gap-2">
            ...
            {viewAllHref ? (
              <Link
                href={viewAllHref}
                className="focus-ring text-meta rounded px-1 py-0.5 text-muted-foreground hover:text-foreground"
              >
                View all →
              </Link>
            ) : null}
...
function ActivityRow({ item }: { item: TimelineRow }) {
  ...
  return (
    <li>
      <Link href={item.href} className="group flex min-w-0 gap-3 px-4 py-2.5 hover:bg-subtle/40">
        <span
          className={cn(
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="text-meta mt-1 flex min-w-0 flex-wrap items-center gap-1.5 text-muted-foreground">
