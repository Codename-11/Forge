...
function authHeaders(): Record<string, string> {
  const token = process.env.HERMES_GATEWAY_TOKEN ?? "placeholder";
  return { authorization: `Bearer ${token}` };
}

export const hermesRunsConnector: DispatchConnector = {
  kind: "hermes-runs",
  ...
  async startRun(input: RunInput): Promise<StartedRun> {
    ...
    const res = await fetch(`${gatewayBase()}/runs`, {
      method: "POST",
      headers,
  ...
  async subscribe(
    ...
    let res: Response;
    try {
      res = await fetch(`${gatewayBase()}/runs/${externalRunId}/events`, {
        method: "GET",
  ...
  async approve(externalRunId: string, choice: string): Promise<void> {
    await fetch(`${gatewayBase()}/runs/${externalRunId}/approval`, {
      method: "POST",
  ...
  async stop(externalRunId: string): Promise<void> {
    await fetch(`${gatewayBase()}/runs/${externalRunId}/stop`, {
      method: "POST",
      headers: authHeaders(),
    }).catch((err) => {
