...
describe("mcp — awareness tools (Stream BA)", () => {
  ...
  it("persists streaming chat drafts and finalizes the same placeholder idempotently", async () => {
    ...
    const started = (await call("chat.startDraft", { threadId: thread.id }, ctx)) as {
      draftId: string;
      recovered: boolean;
    };
    ...
    await expect(call("chat.startDraft", { threadId: thread.id }, ctx)).rejects.toThrow(
      /finalized reply/i,
    );
  });
...
describe("mcp — agent.inbox.list / agent.inbox.ack / agent.inbox.outputStarted", () => {
  ...
  it("agent.inbox.list suppresses chat turns already answered by the agent", async () => {
    const f = await createWorkspaceFixture({ keyPrefix: "INQ" });
    fixtures.push(f);
    const prisma = getPrisma();
    const agent = await prisma.agent.create({
      data: { workspaceId: f.workspace.id, name: "inq", profileKey: "inq", status: "ONLINE" },
    });
    const thread = await prisma.chatThread.create({
      data: {
        workspaceId: f.workspace.id,
  ...
  it("chat.kickThread is a no-op when the latest unacked user message already has a later agent reply", async () => {
    const f = await createWorkspaceFixture({ keyPrefix: "INR" });
    fixtures.push(f);
    const prisma = getPrisma();
    const agent = await prisma.agent.create({
      data: { workspaceId: f.workspace.id, name: "inr", profileKey: "inr", status: "ONLINE" },
    });
    const thread = await prisma.chatThread.create({
      data: {
        workspaceId: f.workspace.id,
