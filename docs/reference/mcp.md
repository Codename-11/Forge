...
| `complete`                  | Close an active/waiting run. `{ runId, summary, completionCommentId?, producedArtifactIds?, verificationResult?, followUps?, confidence?, verdict? }`. `completionCommentId`, when supplied, must identify a live **BODY** comment posted by this run's agent on the same issue after the run started; rolling STATUS comments are rejected. When omitted, Forge creates the final BODY comment from `summary`. Requires an agent-linked key matching the run. Execute enforces issue artifact/checklist gates and applies the configured In Review status after successful completion with audit evidence; Research requires `confidence`; Review requires `verdict`; Discuss is reply-only. Stores `completionMeta` with the Forge run contract version. |
